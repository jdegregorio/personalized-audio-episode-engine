...
For the remainder of the MVP (PR 06 onward), do not request, wait for, or act on GitHub Codex auto-review before merging. A review that is absent, pending, in progress, or incomplete is not a merge blocker. Merge after required local tests, real UAT, required GitHub checks, and the final correctness and simplification passes succeed. Review feedback from closed PRs will be scraped into a separate post-MVP backlog later; do not expand the active MVP PR or delay its merge to process that feedback.

Treat reviewer feedback as evidence to evaluate, not an automatic mandate to add code. Optimize for the approved PoC/MVP: implement findings required for current acceptance criteria, correctness, security, durability, or release safety, but rationale-dismiss suggestions whose benefit is immaterial to the MVP or whose abstraction/configuration/operational cost is disproportionate. When a valid suggestion is useful only after MVP, add a concrete item with expected behavior and acceptance evidence to an explicitly labeled post-MVP follow-up section in `plan.md` instead of expanding the current PR. Prefer the smallest maintainable fix that satisfies the owning requirement; do not introduce speculative frameworks, services, extension points, or generalized machinery merely to satisfy a review comment.

Do not skip or weaken required testing, smoke, UAT, correctness review, or simplification review when tooling or an environment is initially unavailable. Diagnose and independently repair in-scope blockers when access and authority permit, rerun the required evidence, and continue without asking the owner. Stop only when progress genuinely requires owner-only secrets, account/billing or permission changes, physical-device setup, or another action the agent cannot perform; state the exact prerequisite instead of substituting a weaker test.
