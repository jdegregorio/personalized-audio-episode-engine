"""Prepare deterministic, token-bounded TTS prompts for an accepted script."""
...
from audio_engine.tts import TtsPreparationError, open_tts_run, prepare_tts
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        context = open_tts_run(
            args.run_directory,
            settings=settings,
            repo_root=Path(__file__).resolve().parents[1],
        )
        result = prepare_tts(context)
        ...
        return 1
    except (TtsPreparationError, ScriptwritingError, LifecycleError) as error:
        print(_error("tts_preparation_failed", str(error)), file=sys.stderr)
        return 1
