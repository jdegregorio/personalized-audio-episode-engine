...
def _fixed_run_id(profile_id: str, episode_date: date, now: datetime) -> str:
    del now
    return f"{profile_id}_{episode_date.isoformat()}_script_cli"
...
def test_record_script_cli_accepts_and_resumes(
    ...
    accepted_status = record_script_main(["--run", str(run_directory)])
    accepted = json.loads(capsys.readouterr().out)
    resumed_status = record_script_main(["--run", str(run_directory)])
    ...
    assert (run_directory / "transcript.txt").is_file()


def test_record_script_cli_reports_one_repair(
    synthetic_collection_profile_path: Path,
    settings_values: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    ...
    run_directory = _ready_run(synthetic_collection_profile_path, settings_values)
    capsys.readouterr()
    (run_directory / "episode-script.json").write_text("{}", encoding="utf-8")

    result = record_script_main(["--run", str(run_directory)])
    output = json.loads(capsys.readouterr().err)

    assert result == 1
    assert output["status"] == "repair_required"
    assert output["attempt"] == 1
    assert output["errors"][0]["code"] == "schema_error"


def test_record_script_cli_reports_invalid_settings_without_values(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    result = record_script_main(["--run", str(tmp_path)])
    output = json.loads(capsys.readouterr().err)
