...
@pytest.mark.smoke
def test_documented_collection_path_validates_records_and_resumes(
    ...
    assert record_main(["--run", str(run_directory)]) == 0
    ...
    assert state.current_stage == "editorial"
    assert state.collection_validation is not None
    assert state.collection_validation.status == "valid"
